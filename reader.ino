...
1. Scan a valid card. If card is valid, LED will be steady yellow. If card is
invalid, LED will be yellow for a sec then turn red again.

2. Enter password (default 2580). Press the # Key to accept password. Press the
* Key to reset password or entire sequence.
...
void loop() {
  // This waits to make sure that there have been no more data pulses before
  // processing data
  noInterrupts();
  weigand_counter--;
  interrupts();
...
void printBits() {
  ...
  if (cardCode == 00000) {
    ...
    if (enterCode == true) {
      digitalWrite(LED_GREEN, LOW); // Change light to Yellow
      acceptCard = true;
    } else {
      authenticated();
    }
